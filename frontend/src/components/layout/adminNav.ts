...
export interface AdminLink {
  to: string;
  label: string;
  /** Passed to NavLink so "/admin" doesn't match every child route. */
  end?: boolean;
  icon: string;
}
...
/** 24x24 stroke paths, drawn with a shared <svg> wrapper. */
const ICONS = {
...
export const ADMIN_GROUPS: AdminGroup[] = [
  ...
  },
  {
    ...
    links: [
      ...
      { to: "/admin/instructors", label: "Instructors", icon: ICONS.person },
...
export const ADMIN_LINKS = ADMIN_GROUPS.flatMap((group) => group.links);

/** The label for a pathname. Longest match wins, so "/admin/billing" doesn't
 *  resolve to "/admin". */
export function adminLabelFor(pathname: string) {
  const match = ADMIN_LINKS.filter(
